...
string CPU::getInstruction() const {return instruction;}
...
void CPU::setInstruction(const string newString) {instruction = newString;}
...
int CPU::execute(string line, Memory &memObj) {
    ...
    else if(newLine.compare(0, 3, "jpn") == 0) {
 ...
 */
int CPU::jmpInstruction(const string line) {
    int result = -1, regNum;
    if(line[3] == 'r') {
        regNum = line[4] - '0';
 ...
 */
int CPU::jpnInstruction(const string line) {
    int result = -1, regNum;
    if(line[3] == 'r') {
        regNum = line[4] - '0';
        if(regNum >= 1 && regNum <= 5) {
 ...
 */
int CPU::movInstruction(const string line, Memory &memObj) {
 ...
 */
int CPU::movToMem(const string line, int memNum, Memory &memObj) {
 ...
 */
int CPU::movToReg(const string line, int regNum, Memory &memObj) {
    int regNum2, memIndex;
 ...
 * @return 0 if there is no problem -1 otherwise
 */
int CPU::addInstruction(const string line, Memory &memObj) {
 ...
 * @return 0 if there is no problem -1 otherwise
 */
int CPU::subInstruction(const string line, Memory &memObj) {
    int regNum1, regNum2, memIndex;
    regNum1 = line[4] - '0';
    if(regNum1 >= 1 && regNum1 <= 5) {
        if(line[6] == 'r') {
 ...
 * @return 0 if there is no problem -1 otherwise
 */
int CPU::prnInstruction(const string line, Memory &memObj){
    int regNum;
    if(line[3] == 'r') {
        regNum = line[4] - '0';
        if(regNum >= 1 && regNum <= 5)
